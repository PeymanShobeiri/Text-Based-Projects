...
text = '''A cloud computing environment is the most popular choice for workflow execution, as it gives customers on-demand access to computing resources. However, in cloud work- flow scheduling, cloud-native requirements regarding QoS requirements such as monetary cost and execution time should be taken into account. This paper proposes PCP- ACO, an Ant Colony Optimization (ACO) based metaheuris- tic for minimizing the execution cost of a workflow in a cloud with a deadline constraint. First, we distribute the deadline among workflow tasks by using the Partial Crit- ical Path (PCP) concept and build a new graph from the topological sort of the input workflow. Then, using the ACO algorithm, we try to compute the optimal schedule for this newly constructed graph. Several experiments are con- ducted using real scientific workflows, and the cost saving is compared with PSO and IC-PCP algorithms. The exper- imental results show that the proposed algorithm outper- forms other compared algorithms in terms of cost saving.'''
